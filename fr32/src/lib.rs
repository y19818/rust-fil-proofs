...
/// Takes a slice of bytes and returns a vector of Fr -- or an error if either bytes is not a
/// multiple of 32 bytes or any 32-byte chunk overflows and does not contain a valid Fr.
...
/// Takes a slice of Frs and returns a vector of bytes, guaranteed to have a size which is a
/// multiple of 32, with every 32-byte chunk representing a valid Fr.
...
mod tests {
    ...
    #[test]
    #[should_panic]
    fn test_bytes_into_frs_into_bytes_panics() {
        ...
        bytes_into_frs_into_bytes_test(&short_bytes[..]);
    }
}
